...
class DeepBreathingPlotter:
    ...
    def plot(
        ...
        for cycle in result.cycles:
            plot1.addItem(pg.ScatterPlotItem(
                ...
                pen=pg.mkPen(_C_INSP, width=1.5),
                brush=pg.mkBrush(_C_INSP),
            ))
            plot1.addItem(pg.ScatterPlotItem(
                x=[cycle.min_t], y=[cycle.min_v], size=10, symbol="t1",
                pen=pg.mkPen(_C_EXP, width=1.5),
                brush=pg.mkBrush(_C_EXP),
            ))
        ...
        # ── export ────────────────────────────────────────────────────────────
        if output_dir and result.cycles:
            self._export(plot1, result, output_dir)
    ...
    def _export(
        ...
        png_path = output_dir / f"{output_dir.name}_deep_breathing_results.png"
        ImgExp().export_plot(plot1, png_path)

        xlsx_path = ExcelExporter().export_deep_breathing(result, output_dir)
        if png_path.exists():
